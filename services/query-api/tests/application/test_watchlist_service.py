...
class FakeBackfillQueue:
    ...
    def enqueue_symbol_backfill(self, symbol: str) -> None:
        self.enqueued_symbols.append(symbol)


def test_add_symbol_enqueues_a_backfill_task_when_queue_given():
    backfill_queue = FakeBackfillQueue()
    service = WatchlistService(repo=FakeWatchlistRepo(), lookup=FakeLookup({"AAPL"}), backfill_queue=backfill_queue)

    service.add_symbol("user-1", "AAPL")

    assert backfill_queue.enqueued_symbols == ["AAPL"]


def test_add_symbol_does_not_enqueue_on_invalid_symbol():
    backfill_queue = FakeBackfillQueue()
    service = WatchlistService(repo=FakeWatchlistRepo(), lookup=FakeLookup({"AAPL"}), backfill_queue=backfill_queue)
    ...
    assert backfill_queue.enqueued_symbols == []


def test_add_symbol_works_without_a_backfill_queue():
    service = WatchlistService(repo=FakeWatchlistRepo(), lookup=FakeLookup({"AAPL"}))
